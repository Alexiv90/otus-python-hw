...
def get_config_values(init_config, config_path: str) -> ConfigParser:
    config = ConfigParser()
    if isinstance(init_config, dict):
        config.read_dict({"DEFAULT": init_config})
    else:
        config.read(config_path+'/'+init_config)
...
def handle_log_line(line: str) -> NamedTuple:
    ...
    if m := log_regexp.search(str(line)):
...
if __name__ == "__main__":
    ...
    if args.config_path and os.path.exists(args.config_path):
        ...
        config = get_config_values('config.ini', args.config_path)
    main(config)
